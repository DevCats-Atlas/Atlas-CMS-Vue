...
const form = useForm({
    ...
    class_name: props.itemType.className || '',
    fields: props.itemType.selectedFields || [],
    slug_is_multilingual: props.itemType.slug_is_multilingual || false,
});

const submit = () => {
    form.put(`/admin/system/item-types/${props.itemType.id}`);
};

const destroyType = async () => {
    const confirmed = await confirmDialog({
        title: 'Delete item type',
        message: `This will permanently remove "${props.itemType.code}".`,
        confirmLabel: 'Delete',
        intent: 'danger',
    });
    ...
    router.delete(`/admin/system/item-types/${props.itemType.id}`);
};
                ...
                <div class="bg-white dark:bg-gray-800 shadow rounded-xl p-6">
                    ...
                    <form class="space-y-6" @submit.prevent="submit">
                        ...
                        <div class="flex items-center justify-between">
                            <button type="button" class="btn-text-danger" @click="destroyType">
                                Delete type
                            </button>
                            <div class="flex items-center gap-3">
                                ...
                                >
                                    {{ form.processing ? 'Saving...' : 'Update type' }}
                                </button>
                            </div>
                        </div>
                    </form>
