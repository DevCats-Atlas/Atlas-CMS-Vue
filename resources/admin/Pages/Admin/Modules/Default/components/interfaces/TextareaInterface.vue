    ...
    <div class="space-y-4">
        <template v-if="hasTranslations">
            <div v-for="(language, index) in languages" :key="languageKey(language, index)" class="space-y-1">
                ...
                <textarea v-model="model.translations[ensureTranslationKey(language, index)]" class="form-textarea"></textarea>
            </div>
        </template>
        <div v-else class="space-y-1">
            <label v-if="!hideTitle" class="form-label">{{ field.title }}</label>
            <textarea v-model="model.default" class="form-textarea"></textarea>
        </div>
    </div>
</template>
